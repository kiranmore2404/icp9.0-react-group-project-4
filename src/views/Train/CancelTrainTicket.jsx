...
const CancelTrainTicketPage = () => {
  ...
  const handleCancel = () => {
    ...
    if (!storedTicket) {
      toast.error("No ticket found in storage.");
      return;
    }

    const parsedTicket = JSON.parse(storedTicket);

    if (parsedTicket.bookingID === ticketDetails.bookingId) {
      ...
      setTicketDetails({ bookingId: '', cancelReason: '' });
    } else {
      toast.error("Invalid Booking ID. No matching ticket found.");
    }
  };
